# niconico Harajuku-ish UserCSS

現代のニコニコ動画 watch ページを、ニコニコ動画（原宿）風に表示する UserCSS です。
...
2. Stylus の管理画面を開きます。
3. 新規スタイルを作成します。
4. [harajuku.user.css](./harajuku.user.css) の内容をすべて貼り付けます。
5. 保存します。
6. `https://www.nicovideo.jp/watch/*` のページを再読み込みします。
...
`harajuku.user.css` には `@-moz-document domain("nicovideo.jp")` が含まれています。Stylus ではそのまま貼り付けられます。
...
start = css.find('@-moz-document domain("nicovideo.jp") {')
if start >= 0:
    inner = css[start + len('@-moz-document domain("nicovideo.jp") {'):]
...
OS またはブラウザの配色設定に追従します。

- 通常配色: `prefers-color-scheme: light`
- 暗色配色: `prefers-color-scheme: dark`

Stylus 側で切り替えボタンを作る必要はありません。

## 更新

1. [harajuku.user.css](./harajuku.user.css) を編集します。
2. Stylus に貼り付けている内容を差し替えます。
